...
export const actions: ActionTree<FileState, RootState> = {
    ...
    getScanMetadata({ dispatch }, payload: { filename: string }) {
        if (payload !== undefined && payload.filename !== '') {
            ...
            useToast().success(i18n.global.t('Files.ScanMetaSuccess', { filename }).toString())
        }
    },
    ...
    requestMetadata({ commit }, payload: { filename: string }[]) {
        ...
        for (const { filename } of payload) {
            if (messages.length >= 100) {
                getSocketClient().emitBatch(messages)
                messages = []
            }
    ...
    getCreateDir(_, payload) {
        if (payload.error) {
            ...
            useToast().success(<string>i18n.global.t('Files.SuccessfullyCreated', { filename: newPath }))
        }
    },
    ...
    getDeleteDir(_, payload) {
        if (payload.error) {
            ...
            useToast().success(<string>i18n.global.t('Files.SuccessfullyDeleted', { filename: delPath }))
        }
    },
    ...
    async uploadFile({ commit, rootGetters }, payload: { file: File; path: string; root: 'gcodes' | 'config' }) {
        ...
        return new Promise((resolve) => {
            axios
                ...
                .catch(() => {
                    commit('uploadSetShow', false)
                    useToast().error(i18n.global.t('FullscreenUpload.CannotUploadFile').toString())
                    resolve(false)
                })
        })
    },
    ...
    rolloverLog(_, payload) {
        payload.rolled_over.forEach((name: string) => {
            useToast().success(<string>i18n.global.t('Machine.LogfilesPanel.RolloverToastSuccessful', { name }))
        })

        Object.keys(payload.failed).forEach((name: string) => {
            const message = payload.failed[name]

            useToast().error(<string>i18n.global.t('Machine.LogfilesPanel.RolloverToastFailed', { name, message }))
        })

        setTimeout(() => {
            getSocketClient().emit('server.files.get_directory', { path: 'logs' }, { action: 'files/getDirectory' })
        }, 500)
    },
}
