    ...
    <panel :icon="mdiFileDocumentMultipleOutline" :title="$t('History.PrintHistory')" card-class="history-list-panel">
        <v-card-text>
            <v-row>
                ...
                <v-col cols="4" class="offset-4 d-flex align-center justify-end">
                    ...
                    <v-tooltip top>
                        <template #activator="{ props }">
                            <v-btn class="px-2 minwidth-0 ml-3" v-bind="props" @click="addMaintenanceDialog = true">
...
export default defineComponent({
    ...
    computed: {
        ...
        configHeaders(): HistoryListPanelCol[] {
    ...
    methods: {
        ...
        advancedSearch(value: string, search: string) {
            return (
                value != null && search != null && value.toString().toLowerCase().indexOf(search.toLowerCase()) !== -1
            )
        },
