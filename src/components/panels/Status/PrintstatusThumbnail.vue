        ...
        <template v-else>
            <v-container>
                <v-row>
                    ...
                    <v-col v-if="thumbnailSmall" cols="auto" class="pa-2 pl-0">
...
import { debounce, escapePath } from '@/plugins/helpers'
import { FileStateFileThumbnail } from '@/store/files/types'

export default defineComponent({
    name: 'StatusPanelPrintstatusThumbnail',
    mixins: [BaseMixin],
    data() {
        return {
            ...
            if ('thumbnails' in this.current_file && this.current_file.thumbnails.length) {
                ...
                if (thumbnail && 'relative_path' in thumbnail) {
                    ...
                    if (this.current_file.filename.lastIndexOf('/') !== -1) {
                        relative_url = this.current_file.filename.substr(
                            0,
                            this.current_file.filename.lastIndexOf('/') + 1
                        )
                    }
    ...
    methods: {
        calcThumbnailFactor() {
            const bigThumbnail = this.$refs.bigThumbnail as ComponentPublicInstance | undefined
            const thumbnailClientWidth = bigThumbnail?.$el.clientWidth ?? 0
