...
export default defineComponent({
    ...
    data() {
        return {
            ...
            internalCloseCommand: null as number | null,
            checkpointEvent: null as ServerStateEvent | null,
            currentPrompt: [] as ServerStateEventPrompt[],
            // regex that extracts the type and message, omitting the wrapping double quotes of the message (if any)
            promptMessageExp: /^\/\/ action:prompt_(?<type>[^\s]+) *(?<msg>.*)$/,
        }
    },
    computed: {
        events() {
        ...
        activePromptContent(): ServerStateEventPrompt[] {
            const allowedTypes = ['button', 'text', 'button_group_start', 'button_group_end']

            const output = this.activePrompt.filter((event: ServerStateEventPrompt) =>
