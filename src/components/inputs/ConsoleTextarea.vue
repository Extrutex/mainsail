...
export default defineComponent({
    ...
    data() {
        return {
            ...
            gcode: '',
            lastCommandNumber: null as number | null,
            items: [],
        }
    },
    computed: {
        gcodeCommandField(): VTextareaType {
            return this.$refs.gcodeCommandField as VTextareaType
        },
    ...
    methods: {
        ...
        onAutocomplete(e: Event): void {
            ...
            commands.forEach(
                (command) =>
                    (output += `<a class="command font-weight-bold">${command.command}</a>: ${command.help}<br />`)
            )
