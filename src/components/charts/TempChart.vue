...
export default defineComponent({
    ...
    data() {
        return {
            ...
            tooltipPosition: ((point, _params, _el, _rect, size) => {
    ...
    computed: {
        tempchart(): EChartRef | undefined {
            return this.$refs.tempchart as EChartRef | undefined
        },
    ...
    watch: {
        selectedLegends(newVal: Record<string, boolean>) {
            if (this.chart?.isDisposed() === true) return

            this.chart?.setOption({ legend: { selected: newVal } })
        },
        source(newVal: PrinterTempHistoryStateSourceEntry[]) {
