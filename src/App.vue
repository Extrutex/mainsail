...
export default defineComponent({
    ...
    computed: {
        ...
        mainStyle(): Record<string, string> {
            ...
            if (this.naviDrawer && !this.$vuetify.display.mdAndDown) {
        ...
        warningColor(): string {
            return this.$vuetify.theme.current?.colors?.warning?.toString() ?? '#ff8300'
        },
    ...
    watch: {
        ...
        async language(newVal: string) {
            await setAndLoadLocale(newVal)
        },

        customStylesheet(newVal: string | null) {
        ...
        current_file(newVal: string) {
            if (newVal === '') return

            this.$socket.emit('server.files.metadata', { filename: newVal }, { action: 'files/getMetadataCurrentFile' })
        },

        primaryColor(newVal: string) {
            this.$nextTick(() => {
                // update the primary color in all vuetify themes
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const themes: any = this.$vuetify.theme.themes
                const themesObject = 'value' in themes ? themes.value : themes
                for (const name of Object.keys(themesObject)) {
                    themesObject[name].colors.primary = newVal
                }
            })
        },
        ...
        async drawFavicon(val: number): Promise<void> {
            ...
            if ((this.theme?.logo?.show ?? false) && this.sidebarLogo.endsWith('.svg')) {
                ...
                const modifiedSvg = text.replace(
                    /fill="var\(--color-logo, #[0-9a-fA-F]{6}\)"/g,
                    `fill="${this.logoColor}"`
                )
